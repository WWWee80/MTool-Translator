 ...
 * CustomTranslateV2. A second-generation custom endpoint with batching support. Accepts multiple texts per request (JSON or form-encoded payload) and expects a JSON array response.
     * Example Configuration:
         * Endpoint=CustomTranslateV2
         * [CustomV2]
         * Url=http://localhost:5000/api/translate
         * MaxBatchSize=10
         * ResponsePath=data.translations
         * StaticFields=apiKey=my-secret-token
         * Headers=Authorization=Bearer my-secret-token
     * Default JSON request body:
         ```json
         {"from":"ja","to":"en","texts":["こんにちは","世界"],"apiKey":"my-secret-token"}
         ```
     * Expected response (JSON array located via `ResponsePath`):
         ```json
         {"data":{"translations":["Hello","World"]}}
         ```
     * If `ResponsePath` is empty, the endpoint expects the top-level JSON element to be an array (e.g. `["Hello","World"]`).
     * `StaticFields` and `Headers` both accept semicolon separated `key=value` pairs (e.g. `StaticFields=apiKey=my-key;profile=game`).
     * 这里的文档随便看看就好
...
[CustomV2]
Url=                             ;Optional, needed if CustomTranslateV2 is configured
Method=POST                      ;GET or POST
BodyFormat=Json                  ;Json or Form when Method=POST
MaxBatchSize=10                  ;Max texts per request (1-50)
FromField=from                   ;Field or query parameter used for source language
ToField=to                       ;Field or query parameter used for destination language
TextsField=texts                 ;Field or parameter repeated for each text entry
ResponsePath=                    ;Dot path to JSON array (leave empty for top-level array)
StaticFields=                    ;Semicolon separated key=value pairs appended to body/query
Headers=                         ;Semicolon separated Header=Value entries
ContentType=                     ;Override Content-Type header (defaults per body format)
Accept=application/json          ;Accept header value unless overridden in Headers
EnableShortDelay=False           ;Optionally lower per-request delay
DisableSpamChecks=False          ;Optionally disable spam checks for this endpoint
